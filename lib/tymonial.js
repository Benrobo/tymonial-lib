...
class Tymonial {
  ...
  constructor(params) {
    ...
    this.containerElement.classList.add("tymonial-container-element");
  }
  ...
  async #isTemplateAndUserIdExist(template_id, user_id) {
    ...
    if (users.length === 0) {
      throw new TymonialParamsError(
        `No user was found with this ID "${user_id}" `
      );
    }
    ...
    if (filteredTemplate.length === 0) {
      throw new TymonialParamsError(
        `No template was found with this ID "${template_id}" `
      );
    }
  ...
  async #fetch(route, config = {}) {
    ...
    return { req, result, loading: fetchLoading };
  }
  ...
  async #getFeedback(params) {
    ...
    if (feedbacks.length === 0) {
      console.error(
        `It either you dont have a feedback for this template ID "${params?.template_id}"`
      );
      console.error("OR");
  ...
  async init() {
    ...
    const isValidUserData = await this.#isTemplateAndUserIdExist(
      params?.template_id,
      params?.user_id
    );
    ...
    if (isValidUserData) {
      this.containerElement.innerHTML = `
                <div class="tymonial-loader"></div>
            `;
      const userFeedbacks = await this.#getFeedback(params);
      const filteredFeedbacks =
        userFeedbacks.length > 0
          ? userFeedbacks.filter((feed) => feed.published === true)
          : [];

      if (filteredFeedbacks.length > 0) {
        const montageType = Montage(params, filteredFeedbacks);
        this.containerElement.classList.remove("tymonial-container-element");
...
function UiColors(tymonialParams) {
  const tymonialBgColor =
    tymonialParams?.tymonialBgColor === undefined
      ? ""
      : tymonialParams?.tymonialBgColor;

  const cardBgColor =
    tymonialParams?.cardBgColor === undefined ? "" : tymonialParams.cardBgColor;

  const headingColor =
    tymonialParams?.headingColor === undefined
      ? ""
      : tymonialParams.headingColor;

  const subheadingColor =
    tymonialParams?.subheadingColor === undefined
      ? ""
      : tymonialParams.subheadingColor;

  const cardBodyTextColor =
    tymonialParams?.cardBodyTextColor === undefined
      ? ""
      : tymonialParams.cardBodyTextColor;

  const cardUsernameTextColor =
    tymonialParams?.cardUsernameTextColor === undefined
      ? ""
      : tymonialParams.cardUsernameTextColor;

  const cardCareerTextColor =
    tymonialParams?.cardCareerTextColor === undefined
      ? ""
      : tymonialParams.cardCareerTextColor;

  const cardRatingColor =
    tymonialParams?.cardRatingColor === undefined
      ? ""
      : tymonialParams.cardRatingColor;

  const controlsColor =
    tymonialParams?.controlsColor === undefined
      ? ""
      : tymonialParams.controlsColor;

  const controlsBgColor =
    tymonialParams?.controlsBgColor === undefined
      ? ""
      : tymonialParams.controlsBgColor;
...
const Montage = (tymonialParams, feedbacks) => {
  const validHeading =
    tymonialParams?.heading === "" || tymonialParams?.heading === undefined
      ? false
      : true;

  const validSubHeading =
    tymonialParams?.sub_heading === "" ||
    tymonialParams?.sub_heading === undefined
      ? false
      : true;
  ...
  feedbacks.map((list) => {
    ...
    box.innerHTML = `
            ...
            <svg viewBox="0 0 512 512" class="tymonial-quote" ><path d="M464 256h-80v-64c0-35.3 28.7-64 64-64h8c13.3 0 24-10.7 24-24V56c0-13.3-10.7-24-24-24h-8c-88.4 0-160 71.6-160 160v240c0 26.5 21.5 48 48 48h128c26.5 0 48-21.5 48-48V304c0-26.5-21.5-48-48-48zm-288 0H96v-64c0-35.3 28.7-64 64-64h8c13.3 0 24-10.7 24-24V56c0-13.3-10.7-24-24-24h-8C71.6 32 0 103.6 0 192v240c0 26.5 21.5 48 48 48h128c26.5 0 48-21.5 48-48V304c0-26.5-21.5-48-48-48z"/></svg>

            <br />
            <div class="top">
                <div class="tymonial-userImage" style="${profileImgStyle}"></div>
                <p class="body" style="color: ${cardBodyTextColor};">${
      list.message
    }</p>
            </div>
            <br>
            ...
            <div class="bottom">
                <div class="info">
                    <p class="name" style="color: ${cardUsernameTextColor};">${
      list.name
    }</p>
                    <p class="career" style="color: ${cardCareerTextColor};">
                        ${list.userCareer}
                    </p>
                </div>
